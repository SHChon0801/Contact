 ...
 */
class FirstFragment : Fragment(), RecordClickListener, MenuProvider {
        ...
        if (menuItem.itemId == R.id.action_upload) {
            if (myContactViewModel.contactList.isInitialized) {
                ...
                if (userRef.isNullOrEmpty()) {
                    Toast.makeText(context, getString(R.string.error_upload), Toast.LENGTH_SHORT).show()

                } else {
                    val database = Firebase.database(
                        "https://contact-4be59-default-rtdb.asia-southeast1.firebasedatabase.app/").reference
                    myContactViewModel.contactList.value!!.forEach{
                        database.child(userRef).child(it.phone).setValue(it)
                    }
                    Toast.makeText(context, getString(R.string.success_upload), Toast.LENGTH_SHORT).show()
                }
            }
        } else if(menuItem.itemId == R.id.action_downloads) {
